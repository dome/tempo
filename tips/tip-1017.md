...
Validator addresses must be unique among active validators; deactivated addresses
may be reused. Public keys must be globally unique across all validators
(including deleted ones):

- `AddressAlreadyHasValidator`: Reverts if the address is in use by an active validator
...
| Uniqueness | Address only | Address (active only) AND public key (global) |
