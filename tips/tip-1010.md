...
protocolVersion: T1 (classification update in T2)
---
...
**Constraints**:
- Only transactions qualifying for the payment lane (see [Payment Transaction Classification](#payment-transaction-classification)) may exceed the `general_gas_limit`
...
The prefix-only rule allowed an attacker to craft transactions targeting TIP-20 addresses with arbitrary calldata. Such transactions would be classified as payments and bypass the `general_gas_limit`, enabling DoS of the payment lane. The transaction pool and block builder already enforce the stricter calldata check pre-T2, so the attack surface is limited to adversarial block producers. Enshrining the check at the consensus level in T2 closes this vector entirely.
