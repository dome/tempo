...
2. Verify HRP is `tempo`
3. Extract and validate version byte (first byte of data); reject if unrecognized
4. Extract raw address (remaining 20 bytes)
5. If remaining data is not exactly 20 bytes, address is INVALID
...
Bech32m guarantees detection of up to 4 character substitutions. This vector changes 2 characters:

```
                ↓           ↓
Valid:   tempo1qp6z6dwvvc6vq5efyk3ms39une6etu4a9qtj2kk0
Invalid: tempo1qpmz6dwvvc6vq56fyk3ms39une6etu4a9qtj2kk0
  Changes: pos 8 '6'->'m', pos 20 'e'->'6'
  → REJECTED: invalid checksum
```
...
Invalid: tempo1qpmz6dwvve6vq5efyk3us39une6eta4a9qtj2kk0
  Changes: pos 8 '6'->'m', pos 15 'c'->'e', pos 25 'm'->'u', pos 35 'u'->'a'
  → REJECTED: invalid checksum
```
