...
1. `allowAllSelectors = true` maps to `selector_rules = None`; `selectorRules` input is ignored by matching logic and callers SHOULD pass an empty array.
2. `allowAllSelectors = false` maps to `selector_rules = Some(selectorRules)`.
3. `allowAllSelectors = false` with `selectorRules = []` is delete-equivalent for Solidity precompile mutators and disables that target scope.
...
In the raw protocol model, `Some([])` is a no-match scope. In the Solidity precompile mutator path, this form is normalized as delete-equivalent for the target.
...
///      `{ target: address(0), allowAllSelectors: false, selectorRules: [] }`
...
26. Reject selector rules with recipient allowlists when `allowAllSelectors = true`.
...
31. Reject `allowAllRecipients=true` with non-empty `recipients` in `setAllowedCalls`.
32. Reject `allowAllRecipients=false` with empty `recipients` in `setAllowedCalls`.
