...
/// Approximate heap overhead per buffered hashed storage entry.
///
/// `StorageEntry` holds a 32-byte key and a 32-byte value.
const HASHING_BATCH_ENTRY_HEAP_BYTES: usize =
    std::mem::size_of::<B256>() + std::mem::size_of::<U256>();

/// Default heap budget for buffered hashed storage during import.
const DEFAULT_HASHING_BATCH_HEAP_BYTES: usize = 16 * 1024 * 1024 * 1024;

/// Entries per batch before flushing hashed storage to bound heap usage.
const DEFAULT_HASHING_BATCH_SIZE: usize =
    DEFAULT_HASHING_BATCH_HEAP_BYTES / HASHING_BATCH_ENTRY_HEAP_BYTES;
    ...
    InitFromBinaryDump<C>
{
        ...
        N: CliNodeTypes<ChainSpec = C::ChainSpec>,
    {
        ...
        loop {
            ...
            for chunk in (0..pair_count).chunks(self.hashing_batch_size).into_iter() {
                for i in chunk {
                    reader
