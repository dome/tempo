...
The change affects two protocol-level calls on the FeeManager: `collect_fee_pre_tx` and `collect_fee_post_tx`. A new transient storage field tracks whether the current transaction is using two-hop routing and which intermediate token was resolved.
...
2. **Fallback to two-hop.** If the direct pool has insufficient liquidity:
   ...
   - Write the two-hop flag to transient storage.
...
When a fee swap is needed and the two-hop flag is set in transient storage:

1. Re-derive `intermediateToken = TIP20(userToken).quoteToken()`. This is safe because the two-hop flag blocks `completeQuoteTokenUpdate` on the fee token (see Quote Token Immutability Guard below), guaranteeing the value hasn't changed since pre-tx.
...
When the two-hop flag is not set, behavior is unchanged from today.

### Fee Math

Each hop applies the standard M = 9970/10000 rate. The output compounds:

```
out1 = actualSpending × 9970 / 10000
out2 = out1 × 9970 / 10000
     = actualSpending × 9970 × 9970 / (10000 × 10000)
     = actualSpending × 99400900 / 100000000
```

Integer division at each step may produce up to 1 wei of additional rounding dust per hop compared to a single multiplication. This is negligible for stablecoin amounts.
...
|-------|------|-------------|
| `is_two_hop` | `bool` | Whether the current tx uses two-hop fee routing. |

The intermediate token is not stored — it is re-derived in post-tx via `TIP20(userToken).quoteToken()`, which is safe because the two-hop flag prevents `completeQuoteTokenUpdate` from executing. The field is automatically cleared at the end of each transaction (transient storage semantics).

## Quote Token Immutability Guard

A transaction could call `completeQuoteTokenUpdate` on the user's fee token during execution, changing `quoteToken` between pre-tx and post-tx. This would cause post-tx to derive a different intermediate token than what was reserved in pre-tx.

**Mitigation:** When the two-hop flag is set in transient storage, `completeQuoteTokenUpdate` on the fee token MUST revert. The fee token address is already available in transient storage via `getFeeToken()` (TIP-1007). The `completeQuoteTokenUpdate` implementation should check: if the token being updated is the current transaction's fee token AND `is_two_hop` is true, revert.

This restriction only applies within the single transaction that is using two-hop routing. Other transactions in the same block are unaffected because transient storage is per-transaction.
...
| `completeQuoteTokenUpdate` called on fee token during two-hop tx | Reverts. |
...
### Why block `completeQuoteTokenUpdate`?

The intermediate token must be consistent between pre-tx reservation and post-tx execution. Since `quoteToken` is mutable via `completeQuoteTokenUpdate`, and the transaction itself could trigger this update, the simplest safe approach is to block the update when a two-hop reservation is active. This is a narrow restriction (one function, one transaction, only when two-hop is in use).
